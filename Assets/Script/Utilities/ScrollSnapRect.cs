...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    private List<int> planeIndexList = new List<int>();
    ...
    void Start()
    {
        ...
        startingPage = planeIndexList.IndexOf(GameState.currentPlane);
        if (startingPage < 0)
        {
            startingPage = 0;
            GameState.currentPlane.Value = planeIndexList[startingPage];
        }
    ...
    private void SetPagePositions()
    {
        ...
        const int maxPlaneId = 1;
        for (int i = 0; i < _container.childCount; i++)
        {
            RectTransform child = _container.GetChild(i).GetComponent<RectTransform>();
            ShowPlaneAnim plane = child.GetComponent<ShowPlaneAnim>();
            bool isAvailable = plane != null && plane.id >= 1 && plane.id <= maxPlaneId;
            child.gameObject.SetActive(isAvailable);
            if (isAvailable)
            {
                _pages.Add(child);
                planeIndexList.Add(plane.id);
            }
        }
        ...
        if (_horizontal)
        {
            ...
            width = (int)_scrollRectRect.rect.width + 1000;
        ...
        _pagePositions.Clear();
        // iterate through available plane pages and set their positions
        for (int i = 0; i < _pageCount; i++)
        {
            RectTransform child = _pages[i];
    ...
    private void NextScreen()
    {
        ...
        GameState.currentPlane.Value = planeIndexList[_currentPage];
        MenuEventListener.instance.CheckActive();

        //if (GameState.currentPlane < 11)
        //{
        //    //Debug.Log(GameState.currentPlane);
        //}
        CheckShowButton();
    }
    void CheckShowButton()
    {
        bool hasPrevious = _pageCount > 1 && _currentPage > 0;
        bool hasNext = _pageCount > 1 && _currentPage < _pageCount - 1;

        if (prevButton != null)
        {
            prevButton.SetActive(hasPrevious);
        }
    ...
    public void GotoPlane(int planeId)
    {
        if (_pageCount <= 0)
        {
            return;
        ...
        int planeIndex = _currentPage;
        for (int i = 0; i < planeIndexList.Count; i++)
        {
            if (planeIndexList[i] == planeId)
            {
                planeIndex = i;
                break;
            }
